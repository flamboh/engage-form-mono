...
export class ConvexPrivateService extends Context.Tag("ConvexPrivateService")<
  ConvexPrivateService,
  ConvexPrivate
>() {
  static readonly layer = Layer.sync(ConvexPrivateService, () => {
    const convex = new ConvexHttpClient(CONVEX_URL);
    const privateBridgeKey = readRequiredPrivateEnv("CONVEX_PRIVATE_BRIDGE_KEY");

    const withApiKey = <Args extends DefaultFunctionArgs>(args: Omit<Args, "apiKey">) =>
      ({ ...args, apiKey: privateBridgeKey }) as unknown as Args;
...
function readRequiredPrivateEnv(key: string) {
