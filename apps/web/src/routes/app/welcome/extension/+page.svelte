<script lang="ts">
	...
	$effect(() => {
		...
		void (async () => {
			try {
				hasLink = await convexQuery(session, api.extension.hasActiveDeviceToken, {});
			} finally {
				checking = false;
			}
		})();
	...
	async function createToken() {
		...
		creating = true;
		try {
			token = await convexMutation(session, api.extension.createDeviceLinkToken, {
...
<div class="space-y-5">
	<header>
		...
		<p class="mt-1 text-sm text-stone-500">
			Engage Form fills your purchase requests through a Chrome extension. Install it once, paste the token below, and you're set.
		</p>
	</header>
	...
	<ol class="space-y-3 text-sm">
		<li>
			...
			<p class="text-stone-500">Ask your team lead for the install link — it's a single .crx for now.</p>
		</li>
		<li>
		...
		{#if token}
			<li>
				...
				<textarea
					class="field mt-2 h-24 font-mono text-xs"
					readonly
					value={token}
				></textarea>
